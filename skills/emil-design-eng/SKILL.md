...
`@starting-style` and keyframes trigger on DOM insertion, not on resource readiness. When an element depends on an external resource (custom font, video, WebGL canvas), it should stay invisible until the resource is loaded, then fade in. This prevents jarring flashes where unstyled text reflows on font swap or a canvas pops in mid-render.
...
useEffect(() => {
  const timeout = setTimeout(() => setReady(true), 3000)
  // For video: check if already buffered before attaching listener
...
- If the element has sibling overlays (gradients, masks), fade them in together with the same state to avoid white flash artifacts
- Only animate `opacity`; it's GPU-composited and causes no layout or paint
